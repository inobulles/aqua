...
func (b *WgpuBackend) NewFrostPipeline() (*FrostPipeline, error) {
	// TODO Should this be factored out? because we have basically the same thing in TexturePipeline.
	...
	pipeline, err := b.NewPipeline("Frost", frost_shader_src,
		[]wgpu.BindGroupLayoutEntry{
			...
			{ // Background (what will be frostified) texture.
				Binding:    3,
				Visibility: wgpu.ShaderStageFragment,
				Texture: wgpu.TextureBindingLayout{
					Multisampled:  false,
			...
			{ // Background (what will be frostified) sampler.
				Binding:    4,
				Visibility: wgpu.ShaderStageFragment,
				Sampler: wgpu.SamplerBindingLayout{
					Type: wgpu.SamplerBindingTypeFiltering,
	...
	return &FrostPipeline{
		Pipeline:   *pipeline,
		vbo_layout: vbo_layout,
	}, nil
}

func (pipeline *FrostPipeline) Release() {
