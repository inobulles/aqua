...
func CreateInstance(descriptor *InstanceDescriptor) *Instance {
	...
	ref := C.aqua_wgpuCreateInstance(global_ctx.ctx, &desc)
	if ref == nil {
...
func (p *Instance) CreateSurface(descriptor *SurfaceDescriptor) *Surface {
	...
	ref := C.aqua_wgpuInstanceCreateSurface(global_ctx.ctx, p.ref, &desc)
	if ref == nil {
...
func (p *Instance) RequestAdapter(options *RequestAdapterOptions) (*Adapter, error) {
	...
	C.aqua_wgpuInstanceRequestAdapter(global_ctx.ctx, p.ref, opts, C.WGPURequestAdapterCallbackInfo{
...
func (p *Instance) EnumerateAdapters(options *InstanceEnumerateAdapterOptons) []*Adapter {
	...
	size := C.aqua_wgpuInstanceEnumerateAdapters(global_ctx.ctx, p.ref, opts, nil)
	if size == 0 {
		return nil
	}

	adapterRefs := make([]C.WGPUAdapter, size)
	C.aqua_wgpuInstanceEnumerateAdapters(global_ctx.ctx, p.ref, opts, (*C.WGPUAdapter)(unsafe.Pointer(&adapterRefs[0])))
...
func (p *Instance) GenerateReport() GlobalReport {
	...
	C.aqua_wgpuGenerateReport(global_ctx.ctx, p.ref, &r)
...
func (p *Instance) Release() {
	C.aqua_wgpuInstanceRelease(global_ctx.ctx, p.ref)
}
