...
func (p *Queue) OnSubmittedWorkDone(callback QueueWorkDoneCallback) {
	...
	C.aqua_wgpuQueueOnSubmittedWorkDone(global_ctx.ctx, p.ref, C.WGPUQueueWorkDoneCallbackInfo{
...
func (p *Queue) Submit(commands ...*CommandBuffer) (submissionIndex SubmissionIndex) {
	...
	if commandCount == 0 {
		r := C.aqua_wgpuQueueSubmitForIndex(global_ctx.ctx, p.ref, 0, nil)
	...
	r := C.aqua_wgpuQueueSubmitForIndex(global_ctx.ctx, 
		p.ref,
